...
{
    public class AwsDotnetCdkStack : Stack
    {
        internal AwsDotnetCdkStack(Construct scope, string id, IStackProps props = null) : base(scope, id, props)
        {
            ...
            var getAllLambdaSimple = new Function(
                this, "getAllItemsSimpleFunction", new FunctionProps
                {
                    Runtime = Runtime.DOTNET_6,
                    Handler = "GetAllSimple::GetAllSimple.Function::FunctionHandler",
                    Code = Code.FromAsset("./dist/GetAllSimple"),
                    FunctionName = "getAllItemsSimpleFunction",
                    Timeout = Duration.Seconds(10)
                });
            ...
            dynamoTable.GrantReadData(getAllLambdaSimple);
            dynamoTable.GrantDescribeTable(getAllLambdaSimple);
            ...
            var simple = api.Root.AddResource("simple");
            simple.AddMethod("GET", new LambdaIntegration(getAllLambdaSimple, new LambdaIntegrationOptions{
                Proxy = true
            }));
            AddCorsOptions(simple);
